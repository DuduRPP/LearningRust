...
impl TryFrom<&[u8]> for Png {
    ...
    fn try_from(value: &[u8]) -> std::result::Result<Self, Self::Error> {
        value
            ...
            .enumerate()
            .try_for_each(|(i, &num)| {
                if Png::STANDARD_HEADER[i] != num {
                    Err(anyhow!("Invalid header"))
                } else {
                    Ok(())
                }
            })?;
        let mut chunks: Vec<Chunk> = Vec::new();
        let mut start_point: usize = 8;
        while start_point < value.len() {
            let chunk: Chunk = Chunk::try_from(&value[start_point..])?;
            start_point += 4 + 4 + chunk.length() as usize + 4;
            chunks.push(chunk);
        }
        Ok(Png { chunks })
    }
